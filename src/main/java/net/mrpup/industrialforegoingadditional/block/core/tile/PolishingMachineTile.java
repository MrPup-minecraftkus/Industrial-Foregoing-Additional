...
public class PolishingMachineTile extends IndustrialProcessingTile<PolishingMachineTile> {
    ...
    public boolean canIncrease() {
        return this.currentRecipe != null
                && ItemHandlerHelper.insertItem(output, currentRecipe.output.copy(), true).isEmpty()
                && (this.currentRecipe.outputFluid.isEmpty()
                || outputFluid.fillForced(currentRecipe.outputFluid.copy(), IFluidHandler.FluidAction.SIMULATE) == currentRecipe.outputFluid.getAmount());
    }
        ...
        return () -> {
            ...
            if (polishingMachineRecipe != null) {
                if (polishingMachineRecipe.inputFluid1 != null) {
                    ...
                    if (polishingMachineRecipe.outputFluid != null && !polishingMachineRecipe.outputFluid.isEmpty()) {
                        this.outputFluid.fillForced(polishingMachineRecipe.outputFluid.copy(), IFluidHandler.FluidAction.EXECUTE);
                    }

                    ItemStack outputStack = polishingMachineRecipe.output.copy();

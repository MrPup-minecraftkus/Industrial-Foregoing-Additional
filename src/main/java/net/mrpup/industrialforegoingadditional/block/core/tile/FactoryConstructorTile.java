...
public class FactoryConstructorTile extends IndustrialProcessingTile<FactoryConstructorTile> {
    ...
    public FactoryConstructorTile(BlockPos blockPos, BlockState blockState) {
        ...
        this.addTank(this.inputFluid1 = (SidedFluidTankComponent)(new SidedFluidTankComponent("input_fluid", DissolutionChamberConfig.maxInputTankSize, 10 + slotSpacing, 18 + slotSpacing, 1)).setColor(DyeColor.LIME).setTankType(Type.SMALL).setComponentHarness(this).setOnContentChange(() -> this.checkForRecipe()));
        this.addTank(this.inputFluid2 = (SidedFluidTankComponent)(new SidedFluidTankComponent("input_fluid", DissolutionChamberConfig.maxInputTankSize, 56 + slotSpacing, 18 + slotSpacing, 1)).setColor(DyeColor.LIME).setTankType(Type.SMALL).setComponentHarness(this).setOnContentChange(() -> this.checkForRecipe()));

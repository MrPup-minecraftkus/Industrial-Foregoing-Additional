...
public class PolishingMachineEmiRecipe extends CustomEmiRecipeAdditional {
    private final RecipeHolder<PolishingMachineRecipe> recipe;

    public PolishingMachineEmiRecipe(RecipeHolder<PolishingMachineRecipe> recipe) {
        super(recipe.id(), IFEmiPluginAdditional.POLISHING_MACHINE_EMI_CATEGORY, combineIng(fromInputRecipe(((PolishingMachineRecipe)recipe.value()).input1), fromInput(((PolishingMachineRecipe)recipe.value()).input2), fromInput(((PolishingMachineRecipe)recipe.value()).inputFluid1)), fromOutputRecipe((ItemStack)((PolishingMachineRecipe)recipe.value()).output.orElse(ItemStack.EMPTY), (FluidStack)((PolishingMachineRecipe)recipe.value()).outputFluid.orElse(FluidStack.EMPTY)));
        this.recipe = recipe;
    }
    ...
    public void addWidgets(WidgetHolder widgets) {
        for (int i = 0; i < recipe.value().input1.size(); ++i) {
            widgets.addSlot((EmiIngredient) this.getInputs().get(i), 49, 21);
        }

        for (int i = 0; i < recipe.value().input2.size(); ++i) {
            widgets.addSlot((EmiIngredient) this.getInputs().get(recipe.value().input1.size() + i), 49, 44);
        }

        widgets.addSlot(EmiIngredient.of(List.of((EmiStack)this.getOutputs().get(0))), 118, 15).recipeContext(this);
        widgets.addTank((EmiIngredient)this.getInputs().get(this.getInputs().size() - 1), 72, 34, 14, 15, 1000).drawBack(false);
        ...
        widgets.addDrawable(0, 0, 0, 0, (draw, mouseX, mouseY, delta) -> {
            ...
            SlotsScreenAddon.drawAsset(draw, Minecraft.getInstance().screen, DefaultAssetProvider.DEFAULT_PROVIDER, 28, 22, 0, 0, 1, UpgradedConstructorTile::getSlotPos, i -> ItemStack.EMPTY, true, i -> new Color(DyeColor.BLUE.getFireworkColor()), i -> true, 1);
            SlotsScreenAddon.drawAsset(draw, Minecraft.getInstance().screen, DefaultAssetProvider.DEFAULT_PROVIDER, 28, 45, 0, 0, 1, UpgradedConstructorTile::getSlotPos, i -> ItemStack.EMPTY, true, i -> new Color(DyeColor.MAGENTA.getFireworkColor()), i -> true, 1);
            ...
            int consumed = recipe.value().processingTime * PolishingMachineConfig.powerPerTick;

...
public class PolishingMachineCategory implements IRecipeCategory<PolishingMachineRecipe> {
    ...
    public Component getTitle() {
        return Component.translatable(ModuleCoreAdditional.POLISHING_MACHINE.getKey().get().getDescriptionId());
    }
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, PolishingMachineRecipe recipe, IFocusGroup focuses) {
        ...
        for (int i = 0; i < recipe.inputFirst.length; ++i) {
            Ingredient.Value value = recipe.inputFirst[i];
        ...
        for (int i = 0; i < recipe.inputSecond.length; ++i) {
            Ingredient.Value value = recipe.inputSecond[i];
        ...
        if (!recipe.inputFluid1.isEmpty()) {
            builder.addSlot(RecipeIngredientRole.INPUT, 73, 35)
                    ...
                    .addIngredient(ForgeTypes.FLUID_STACK, recipe.inputFluid1);
        }

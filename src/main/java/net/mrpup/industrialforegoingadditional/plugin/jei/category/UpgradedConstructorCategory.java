...
public class UpgradedConstructorCategory implements IRecipeCategory<UpgradedConstructorRecipe> {
    ...
    public Component getTitle() {
        return Component.translatable(ModuleCoreAdditional.UPGRADED_CONSTRUCTOR.getKey().get().getDescriptionId());
    }
    ...
    public IDrawable getIcon() {
        return guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK,
                new ItemStack(ModuleCoreAdditional.UPGRADED_CONSTRUCTOR.getKey().get()));
    }
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, UpgradedConstructorRecipe recipe, IFocusGroup focuses) {
        ...
        if (recipe.inputFluid1 != null && !recipe.inputFluid1.isEmpty()) {
            builder.addSlot(RecipeIngredientRole.INPUT, 48, 35).setFluidRenderer(1000L, false, 12, 13).setOverlay(this.smallTank, 0, 0).addIngredients(ForgeTypes.FLUID_STACK, Arrays.asList(recipe.inputFluid1));
        }

        if (!recipe.output.isEmpty()) {
            ItemStack stack = (ItemStack)recipe.output.copy();

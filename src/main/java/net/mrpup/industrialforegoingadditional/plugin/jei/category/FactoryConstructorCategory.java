...
public class FactoryConstructorCategory implements IRecipeCategory<FactoryConstructorRecipe> {
    ...
    public Component getTitle() {
        return Component.translatable(ModuleCoreAdditional.FACTORY_CONSTRUCTOR.getKey().get().getDescriptionId());
    }
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, FactoryConstructorRecipe recipe, IFocusGroup focuses) {
        ...
        for (int i = 0; i < recipe.input.length; ++i) {
            ...
            Ingredient.Value value = recipe.input[i];

            if (value != null) {
                Ingredient ingredient = Ingredient.fromValues(java.util.stream.Stream.of(value));

                builder.addSlot(RecipeIngredientRole.INPUT, 24 + (Integer) FactoryConstructorTile.getSlotPos(i).getLeft(), 11 + (Integer)FactoryConstructorTile.getSlotPos(i).getRight())
                        .addIngredients(ingredient);
            }
        }

        if (recipe.inputFluid1 != null && !recipe.inputFluid1.isEmpty()) {
            builder.addSlot(RecipeIngredientRole.INPUT, 73, 35).setFluidRenderer(1000L, false, 12, 13).setOverlay(this.smallTank, 0, 0).addIngredient(ForgeTypes.FLUID_STACK, recipe.inputFluid1);
        }

        if (recipe.inputFluid2 != null && !recipe.inputFluid2.isEmpty()) {
            builder.addSlot(RecipeIngredientRole.INPUT, 23, 35).setFluidRenderer(1000L, false, 12, 13).setOverlay(this.smallTank, 0, 0).addIngredient(ForgeTypes.FLUID_STACK, recipe.inputFluid2);
        }

        if (!recipe.output.isEmpty()) {
            ItemStack stack = (ItemStack)recipe.output;
        ...
        if (recipe.outputFluid != null && !recipe.outputFluid.isEmpty()) {
            builder.addSlot(RecipeIngredientRole.OUTPUT, 142, 17).setFluidRenderer(1000L, false, 12, 50).setOverlay(this.bigTank, 0, 0).addIngredient(ForgeTypes.FLUID_STACK, (FluidStack)recipe.outputFluid);
        }
